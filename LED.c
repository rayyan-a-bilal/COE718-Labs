...
void LED_Init (void) {
  ...
  LPC_SC->PCONP     |= (1 << 15);            /* enable power to GPIO & IOCON  */
	...
	LPC_GPIO1->FIODIR |= (1 << 28);						 /* pin 28(LED) for up */
	LPC_GPIO1->FIODIR |= (1 << 29);						 /* pin 29(LED) for down */
	LPC_GPIO1->FIODIR |= (1 << 31);						 /* pin 28(LED) for left */

	LPC_GPIO2->FIODIR |= 0x0000007C;           /* LEDs on PORT2 are output      */
	
	LPC_GPIO2->FIODIR |= (1 << 2);						 /* pin 2 for right */
	LPC_GPIO2->FIODIR |= (1 << 3);						 /* pin 3 for select */
}
